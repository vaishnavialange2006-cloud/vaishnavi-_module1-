...
public class CRC {
    public static void main(String[] args) {
        ...
        Scanner scan = new Scanner(System.in)){
            ...
            for(int i=0;i<size; i++){
                System.out.println("Enter bit"+ (size-1)+":");
                data[i] = scan.nextInt();
            }
    ...
    static int[]divideDataWithDivisor(int oldData[],int divisor[]){
        ...
        System.arraycopy(data,0,rem,0,oldData.length);
...
static int exorOperation(int x, int y){
    ...
    static void receiveData(int data[],int divisor[]){ 
       ...
       for(int i=0; i<rem.length;i++){
        if(rem[i] != 0){
            System.out.println("Currupted data receiver...");
            return;
        }
       }
       System.out.println("Data received without any array.");
    }
}
